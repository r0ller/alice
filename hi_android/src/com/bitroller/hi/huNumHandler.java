...
public class huNumHandler {
    ...
    private huNumHandler(String number, String suffix){
        ...
        if(number.length()<5) {//TODO: support longer numbers
            for (int i = 0; i < number.length(); ++i) {
                if (number.length() - i == 4) {
                    if (number.charAt(i) != '0' && number.charAt(i) != '1')
                        text += convertDigit(number.charAt(i)) + "ezer";
                    else if (number.charAt(i) == '1') text += "ezer";
                } else if (number.length() - i == 3) {
                    if (number.charAt(i) != '0' && number.charAt(i) != '1')
                        text += convertDigit(number.charAt(i)) + "száz";
                    else if (number.charAt(i) == '1') text += "száz";
                } else if (number.length() - i == 2) {
                    if (number.charAt(i + 1) == '0') {
                        text += convertDecimalNulled(number.charAt(i));
                    } else {
                        text += convertDecimalNotNulled(number.charAt(i));
                    }
                } else if (number.length() - i == 1) {
                    if (number.length() > 1 && number.charAt(i) != '0' || number.length() == 1)
                        text += convertDigit(number.charAt(i));
                }
            }
