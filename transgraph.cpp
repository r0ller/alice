...
std::string transgraph::transcript(std::map<std::string,std::string>& functors,const std::map<unsigned int,std::pair<std::string,unsigned int>>& node_functor_map,const std::string& target_language,
	std::vector<std::tuple<unsigned int,std::string,std::string,unsigned int,unsigned int,std::string,unsigned int,std::string,std::string>>& dependency_path,std::set<std::string> features_to_inherit,const std::string& ref_id,const unsigned int level,const std::string& parent_functor,
			...
			for(auto&& i:arguments){
				...
				if(*semantic_dependency=="CON"){
					...
					if(argument_script!=argument_scripts.end()){
						argument_script->second+=i.second->transcript(functors,node_functor_map,target_language,dependency_path,features_to_inherit,ref_id,level+1,functor.first,functor.second,std::stoi(d_counter_field->second.field_value));
					}
					else{
						initial_argscript=i.second->transcript(functors,node_functor_map,target_language,dependency_path,features_to_inherit,ref_id,level+1,functor.first,functor.second,std::stoi(d_counter_field->second.field_value));
...
std::string transgraph::get_tags(query_result *functor_tag_entries,morphan_result *morphan,std::set<std::string>& features_to_inherit,const std::string& ref_id) const{
	std::string tags;
